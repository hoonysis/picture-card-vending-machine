...
function openRenameInterceptor(file, remainingCount = 0) {
    ...
    // 1. Update UI
    const srOriginal = document.getElementById('interceptor-original');
    const srCount = document.getElementById('interceptor-count');
    const srBadge = document.getElementById('interceptor-remain-badge'); // [NEW] Inline Badge
    const srInput = document.getElementById('interceptor-input');
    const srContainer = document.getElementById('rename-interceptor-modal');

    if (srOriginal) srOriginal.textContent = file.name;

    // Header Count (Existing)
    if (srCount) {
        if (remainingCount > 0) {
    ...
    // [NEW] Main Screen Badge Update (Requested by User)
    const mainBadge = document.getElementById('main-remain-badge');
    if (mainBadge) {
        if (remainingCount > 0) {
            mainBadge.textContent = `(${remainingCount}장 남음)`;
    ...
    // 3. Update Main Preview
    const reader = new FileReader();
    reader.onload = (e) => {
        const img = document.getElementById('interceptor-preview');
        if (img) {
    ...
    if (srInput) {
        srInput.focus();
        srInput.select();
        isRenaming = false; // Allow F4 -> Upload
    }

    // Ensure old modal is closed
    const oldModal = document.getElementById('rename-interceptor-modal');
...
function confirmInterceptorName(skipFocus = false) {
    const input = document.getElementById('interceptor-input');
    if (!input) return;
